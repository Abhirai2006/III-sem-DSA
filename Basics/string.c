...
#define MAX_STR 1000   /* max length for input string */
#define MAX_PAT  200   /* max pattern length */
#define MAX_REP  200   /* max replacement length */

/* replace all non-overlapping occurrences of pat in str with rep */
void replace_all(const char *str, const char *pat, const char *rep, char *out, size_t out_size) {
    size_t n = strlen(str);
    size_t p = strlen(pat);
    size_t r = strlen(rep);

    if (p == 0) { /* empty pattern: just copy input */
        strncpy(out, str, out_size - 1);
        out[out_size - 1] = '\0';
        return;
    }

    size_t i = 0; /* index in input str */
    size_t j = 0; /* index in output out */

    while (i < n) {
        /* check if pattern matches starting at str[i] */
        if (i + p <= n && strncmp(&str[i], pat, p) == 0) {
            /* append replacement if it fits */
            if (j + r >= out_size) break; /* avoid overflow */
            strncpy(&out[j], rep, r);
            j += r;
...
int main(void) {
    ...
    char pat[MAX_PAT];
    char rep[MAX_REP];
    char ans[MAX_STR * 2]; /* output buffer (allow growth) */
